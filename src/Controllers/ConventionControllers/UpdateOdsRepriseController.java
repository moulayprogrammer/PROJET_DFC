...
public class UpdateOdsRepriseController implements Initializable {
    ...
    public void Init(OdsReprise odsReprise) {

        tfNumero.setText(odsReprise.getNumber());
        dpDate.setValue(odsReprise.getDate());

        this.odsReprise = odsReprise;
    }
    ...
    private boolean update(OdsReprise odsReprise){
        boolean ins = false;
        try {
            ins = operation.update(odsReprise , this.odsReprise);
        }catch (Exception e){
            e.printStackTrace();
        }
        return ins;
